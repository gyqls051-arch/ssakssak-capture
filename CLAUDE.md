...
- **12차** (2026-07-09): **Windows 전용 확정 + 핸드오프 + v1.0.4 커밋** — macOS 포팅 검토 결과 "맥은 별도 네이티브 앱으로" 결정(비-자명 결정 참조). README에 Windows 전용 명시 + 인코더 설명 최신화, `.gitignore`의 `CLAUDE.md` 줄 제거(미결정 해소), `HANDOFF.md`를 v1.0.4 기준으로 전면 갱신. Phase 1+2 산출물 전체 커밋. **빌드/실기테스트/릴리즈는 여전히 미완** — 다음 작업자는 HANDOFF.md부터.
