...
> 🪟 **Windows 전용** (Windows 10/11). 코어가 Win32/DWM/gdigrab 위에 만들어져 있어 macOS·리눅스는 지원하지 않으며 지원 계획도 없습니다.

## 다운로드

[**최신 인스톨러 다운로드 →**](https://github.com/gyqls051-arch/ssakssak-capture/releases/latest)
...
- 하드웨어 인코더가 없으면 H.264 SW 폴백 (h264_mf → libopenh264 → mpeg4), 시작 실패 시 다음 인코더로 자동 재시도
...
### 1. 인스톨러 (추천)
[Releases](https://github.com/gyqls051-arch/ssakssak-capture/releases/latest) 에서 `Setup_SsakKimchiCapture_X.X.X.exe` 다운로드 → 더블클릭.

### 2. 포터블 ZIP
[Releases](https://github.com/gyqls051-arch/ssakssak-capture/releases/latest) 에서 `싹싹김치 캡처.zip` → 압축 풀고 `싹싹김치 캡처.exe` 실행.

### 3. 소스에서 직접
```powershell
git clone https://github.com/gyqls051-arch/ssakssak-capture.git
cd ssakssak-capture
...
## 기여 / 버그 제보
- 버그 제보: [Issues](https://github.com/gyqls051-arch/ssakssak-capture/issues/new/choose)
